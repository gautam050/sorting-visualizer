function Selection_sort() {
    ...
    for (let i = 0; i < array_size - 1; i++) {
        ...
        for (let j = i + 1; j <= array_size; j++) { // Change here
            if (j < array_size) {
                div_update(divs[j], div_sizes[j], "yellow"); //Color update
            }

            if (j < array_size && div_sizes[j] < div_sizes[index_min]) { // Check condition for j
                if (index_min != i) {
        ...
        div_update(divs[i], div_sizes[i], "green"); //Color  //Gautams update
    }
